...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  <xsl:output method="xml" encoding="UTF-8" byte-order-mark="no" indent="yes" omit-xml-declaration="no" cdata-section-elements="skipped failure "/>
  <xsl:template match="/">
    <testsuites>
          ...
          <xsl:for-each select="collection/test">
                <testcase>
                  ...
                  <xsl:if test="@result='Fail'">
                    <failure>
                      <xsl:attribute name="type"><xsl:value-of select="failure/@exception-type"/></xsl:attribute>
